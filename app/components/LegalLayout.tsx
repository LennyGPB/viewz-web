...
export default function LegalLayout({ title, kicker, sections }: { title: string; kicker: string; sections: LegalSection[] }) {
  return <main className="legal-shell"><div className="ambient ambient-one" />
    <header className="legal-header"><div className="header-inner"><Link href="/" className="wordmark">ViewZ</Link><nav><Link href="/cgu">CGU</Link><Link href="/politique-confidentialite">Confidentialité</Link><Link href="/securite-enfants">Sécurité des enfants</Link><Link href="/suppression-compte">Supprimer mon compte</Link></nav></div></header>
